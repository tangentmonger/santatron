...
void setup() {
    ...
    digitalWrite(COMMON_LED, LOW); //off
...
void loop() {
  ...
  waitForGoButton();
    ...
    char elfQuotient[30];
    strcpy(elfQuotient, "ELF quotient: ");
    char quotient[3];
    itoa(elfs[currentSelection], quotient, 10);
    strcat(elfQuotient, quotient);
    strcat(elfQuotient, "%");
    resultLightOn();
    printText(text, elfQuotient);
    resultLightOff(PRINT_TIME);
    buttonStatus = digitalRead(GO_BUTTON);
} 
...
char* formatELFQuotient(){
    //todo:malloc/free, same for large text
    char text[30];
    strcpy(text, "ELF quotient: ");
    //char quotient[3];
    //itoa(elfs[currentSelection], quotient, 10);
...
//Glow red or green (depending on ELF quotient) 
void resultLightOn(){
    int colour;
...
//Wait for roughly as long as it takes to print, then turn lights off
void resultLightOff(int time){
